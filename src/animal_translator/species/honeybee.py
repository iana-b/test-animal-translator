...
Расстояние и направление вычисляются по опубликованным уравнениям.
Коэффициенты берутся из data/knowledge/honeybee.json и в модуле не дублируются.
"""
...
def translate(observation: dict[str, Any]) -> Result:
    ...
    # но количественной модели для них не опубликовано.
    if dance_type in ("tremble", "stop_signal"):
        return Result(
            ...
            warnings_ru=[
                "Числовая уверенность не выводится: количественной модели для этого сигнала "
                "не опубликовано, а оценка контекста здесь опирается на эксперимент, а не на расчёт."
            ],

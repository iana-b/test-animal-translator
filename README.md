...
Приложение построено на **FastAPI**. Разбор наблюдения вызывается и со страниц,
и по HTTP: логика одна и та же, отличается только форма ответа — разметка или
JSON. Схема OpenAPI и интерактивная документация генерируются из моделей, писать
...
Страница вида целиком: как устроен разбор, методика с измеренными величинами,
мифы и источники с выборками, лицензиями и статусом доступа.

![База знаний](docs/screenshots/09-knowledge.png)

Документация API генерируется из моделей Pydantic, запросы выполняются прямо
из браузера:
...
173 теста. Проверяются не только ветвления кода, но и содержание базы: что

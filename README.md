...
Прототип, который разбирает наблюдение за сигналом животного и выдаёт вероятную
трактовку вместе с уверенностью, альтернативами и ссылками на исследования.

Ключевое свойство: **приложение отказывается отвечать там, где данных нет**, и
различает четыре причины отказа — не хватило наблюдения, сигнал этой информации
не несёт, правило здесь не работает, наука такого ещё не установила.

Все числа в расчётах возводятся к опубликованным работам. Величины, которые
измерены, помечены как взятые из статьи; величины, подобранные автором, помечены
как эвристики — и то и другое видно и в базе знаний, и в интерфейсе.
...
Пчела: расстояние и направление вычислены по опубликованным уравнениям,
уверенность 85%, под каждым шагом — идентификатор источника.

![Разбор танца пчелы](docs/screenshots/02-bee-full.png)

Собака: по одному звуку контекст не определяется. Приложение не молчит —
показывает всё распределение и говорит, какое наблюдение помогло бы.

![Отказ у собаки](docs/screenshots/04-dog-refusal.png)

Та же запись, но указана обстановка: распределение сузилось, ответ выдан.

![Собака с контекстом](docs/screenshots/05-dog-context.png)

Слон: значение подтверждено проигрыванием записи живым слонам.

![Тревожный рёв слона](docs/screenshots/06-elephant.png)

Кашалот: структура коды разобрана, значения не существует ни одного.

![Кашалот, кода](docs/screenshots/07-whale.png)

Тот же вид, но эхолокационные щелчки — единственный его сигнал, функция которого
установлена измерениями. Здесь появляется и число: 81% времени погружения занято
регулярными щелчками. Под ним подписано, что это измеренная доля, а не уверенность
в трактовке.

![Кашалот, щелчки](docs/screenshots/08-whale-clicks.png)

База знаний вида: источники, статус доступа, лицензии, опубликованные поправки.
...
| **Кашалот** | `structure_without_semantics` | Отказ по существу: структура сигнала разбирается, значения не существует |
...
Хранилище — четыре JSON-файла в `data/knowledge`, по одному на вид, около 1600
строк суммарно. Не SQL: файл открывается редактором и читается глазами, а правка
коэффициента видна в истории git как строка.
...
**Пчела.** `R² × протокол × диапазон × калибровка`, ограничено потолком вида 0.95.
Первый множитель — качество подгонки модели из статьи, остальные три —
эвристические штрафы, помеченные как таковые. Отдельной строкой показывается
...
120 тестов. Проверяются не только ветвления кода, но и содержание базы: что
...
Набор проверялся мутациями: коэффициенты, пороги и типы отказов подменялись по
одному, и каждая подмена роняла тесты.
...
- **King L.E., Soltis J., Douglas-Hamilton I., Savage A., Vollrath F.** (2010). Bee threat elicits alarm call in African elephants. *PLoS ONE 5: e10346*. [doi.org/10.1371/journal.pone.0010346](https://doi.org/10.1371/journal.pone.0010346)
  ...
  <br>На звук пчёл семьи уходили в 14 случаях из 15 (93%) против 6 из 13 (46%) на белый шум, средняя дистанция ухода 71.67 м против 32.3 м. Проигрывание самого пчелиного рёва вызвало уход у 6 семей из 10 против 2 из 10 на контрольный рёв. Тряска головой усиливалась значимо (p = 0.002)
...
- **Herbst C.T., Stoeger A.S., Frey R., Lohscheller J., Titze I.R., Gumpenberger M., Fitch W.T.** (2012). How low can you go? Physical production mechanism of elephant infrasonic vocalizations. *Science 337: 595–599*. [doi.org/10.1126/science.1219712](https://doi.org/10.1126/science.1219712)
  <br>Доказательность: сильная · полный текст закрыт
  <br>Инфразвук возникает обычными колебаниями голосовых складок под действием потока воздуха, а не отдельными мышечными сокращениями

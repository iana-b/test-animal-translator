...
SQLite для запросов, 177 тестов.
...
только форма ответа — разметка или JSON. Документация и схема генерируются из
моделей Pydantic.
...
Источник правды — JSON в `data/knowledge`. Для запросов поперёк видов из него
собирается SQLite: файл базы не хранится в репозитории, его создаёт скрипт.
...
Три таблицы заполнены только для собаки, и это отражает суть проекта: о разных
видах известно разное. У собаки измерения естественно ложатся в таблицу — шесть
контекстов, доли распознавания, матрица путаницы. У пчелы то же место занимают
коэффициенты уравнений, у слона — уровни доказательности, у кашалота — признаки
коды. Сводить их в общую таблицу значило бы придумать сходство, которого нет.
...
Сводка по библиографии — сколько источников на вид, какая доля доступна целиком и
за какие годы:
...
Следующие два запроса — про собаку: матрица путаницы есть только у неё.
...
Ещё три готовых запроса — в `scripts/build_db.py`: открытые источники по всем
видам, статьи с опубликованными поправками и полная сводка. Все выполняются
командой `--demo` и покрыты тестом, который следит, что каждый из них работает и
возвращает строки.
...
Слон. Тряска головой отличает тревогу на пчёл от тревоги на людей. Значение
проверено экспериментом: запись проигрывали диким слонам, и 6 семей из 10 ушли
против 2 из 10 на контрольную запись.
...
177 тестов. Проверяются не только ветвления кода, но и содержание базы: что
